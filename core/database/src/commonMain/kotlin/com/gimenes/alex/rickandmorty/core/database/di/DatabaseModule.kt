 ...
 * here - [characterDao][com.gimenes.alex.rickandmorty.core.database.CharacterDao] was the first
 * of these (issue #5); the game-state DAOs
 * ([quizResultDao][com.gimenes.alex.rickandmorty.core.database.QuizResultDao],
 * [streakDao][com.gimenes.alex.rickandmorty.core.database.StreakDao]) are added alongside it
 * here (issue #6).
 */
val databaseModule: Module = module {
    includes(platformDatabaseModule)
    single { get<RickAndMortyDatabase>().characterDao() }
    single { get<RickAndMortyDatabase>().quizResultDao() }
    single { get<RickAndMortyDatabase>().streakDao() }
}
